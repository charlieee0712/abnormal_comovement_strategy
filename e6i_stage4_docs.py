...
def main():
    ...
    LS = ['# E6i line_state_proposal（执行端草稿；状态由用户定，规划 session 复核）\n',
          ...
          '**E6i proposal 事前登记的降级条件**（`exec_briefs/E6i_proposal.md` §1.4 线状态表"因子端迭代"行，原文）：'
          '"若本轮八族在推导段测量级诊断全部 |ρ| ≤ 0.08 且角色级对同人数核心中位 ≤ 0 → 下轮转 carried，且只能按"族 × 域"写否定"。'
          '两半是"且"：K 槽位（主方向 + 部分混入，1134 个描述符）推导段对同人数核心中位 +0.187 / +0.379（part1a〔Q2-OBJ〕）→ 后半不成立 → 条件未触发；'
