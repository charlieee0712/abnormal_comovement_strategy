# E2 —— 生产损害表：六候选在 {旧/新时点} × {未/已复权} 四格下的分解

用途：权重只算一次（与交付脚本逐 config 同构），引擎调四次，把每个候选每段的 DEV-net 变化拆成时点项、复权项、交互项。同时给出三条引擎腿（cond / tvol / reversal 单因子）、裸底池、T 日探测器在池内的表现，供规划 session 判断哪些既有结论要复审。**旧口径组合必须逐格复现 phase3 的 24 个锚点，这是 phase 链路在 E1 之后的接线自检。** 前置：`E1_REPORT.md` 5/5 PASS、commit `cb9fea2`。先读 `00_协议.md`。

## 0. 背景（读懂再动手）
- E1 之后 `C.compute_calendar_pnl` 默认 `(exec_lag=1, adjust=True)`。交付脚本 / phase 脚本仍按默认值调用，所以它们的锚点自检现在**按设计会 FAIL**，不要去跑它们、不要改它们。
- 本步脚本显式传 `exec_lag` / `adjust`，四个组合记为：`L0R` = (0, False) 旧口径；`L1R` = (1, False) 只改时点；`L0A` = (0, True) 只改复权；`L1A` = (1, True) 新口径（正式）。
- 锚点两级：**硬锚点** = phase3 `summary.csv` 的 24 格（六候选 × 四段，即交付脚本 ANCHORS，E0 已用现引擎复现到 |d|0.000）→ 任一 FAIL 即 `[ABORT]`；**软锚点** = phase1 `summary.csv` 的 `cond` / `rev_solo` 各四段 → FAIL 只打 WARN 不中止（它们的构造在 phase1 里的细节可能与本脚本略有不同，规划 session 来判）。
- 窗口不变（`PERIODS` 四段，数据到 2026-03-27）。样本延长是另一步，混进来会污染归因。
- 权重与持仓不依赖引擎，所以本步**不重出交付池**。
- E1 已知事实（供你对照量级）：T 日探测器在全市场池上 旧口径 +29.5%/NW 9.0 → 新口径 −6.2%/NW −2.0；池等权自比在两种时点下都 ≈0。

## 1. 起点确认（任一不符 → 停，写 BLOCKERS）
1. `E1_REPORT.md` 结论 5/5 PASS；47 上 `git log -1 --oneline` = `cb9fea2`；`git status --short` 仍只有那 3 个已知 untracked。
2. `python -c "import comprehensive_factor_diagnosis as C; print(C.compute_calendar_pnl.__defaults__)"` = `(5, 6, 1, True)`。
3. 四个分段缓存在（同 E0 第 5 项）。
4. `ps` 无本项目残留任务；负载 <5。

## 2. 新建 `e2_decomposition.py`（project_core 根目录，import-only，只读地基）

完整脚本如下。**pipeline 段（数据 → 特征 → 池 → 中性化 → 六候选 mask）逐字取自交付脚本，不得改动；允许修语法类小错，不得改 CFGS / COMBOS / ANCHORS / TOL。**
...
跑法（预计 8–15 分钟：pipeline 与交付脚本同量级 ≈4 分钟，另加 11 cfg × 4 组合 × 4 段 = 176 次引擎调用）：
```bash
cd /mnt/sda2/lichenchen/code/project_core
python -c "import ast; ast.parse(open('e2_decomposition.py').read()); print('syntax OK')"
nohup taskset -c 96-191,288-383 python -u e2_decomposition.py > /tmp/E2_decomp.log 2>&1 &
disown
```
超过 30 分钟无 `[ALLDONE]` / `[ABORT]` → 看 `ps` 后报告，不重复启动。

## 3. 验收
- **G2-1 硬锚点**：日志 24 行 `[CHECK HARD] … OK`，且 `[SELF-CHECK] HARD 24/24 PASS`。E1 的 T3 已证 (0,False) 路径逐格等于旧函数，所以这 24 格应当 `|d|0.0000`；若出现 0 < |d| < 0.02 也算过但要在 REPORT 单独列出并说明。任一 FAIL → 脚本自行 `[ABORT]`，REPORT 写 BLOCKERS（哪格、差多少、你认为的原因），**不改锚点、不改 TOL、不改 pipeline**。
- **G2-2 软锚点**：8 行 `[CHECK SOFT]`，OK 或 WARN 都原样摘进 REPORT；WARN 不阻塞。
- **G2-3 产物**：`summary.csv`（176 行 = 11 cfg × 4 段 × 4 组合）、`check.csv`（32 行）、`decomp.csv`（44 行）、`log.txt` 含三张最终表。
- **G2-4 合理性（只报告，不断言）**：`detS_pool` 的 `d_timing` 应显著为负（旧口径把 T 日盘中涨记进组合）；`pool0_DEV` 给出裸底池在新口径下的四段 net 与 NW。把这两行单独摘出。

## 4. 交回
- 47 上 `git add e2_decomposition.py && git commit -m "E2 分解表脚本: 六候选+三腿+裸底池+探测器 x {exec_lag 0/1}x{adjust F/T}, 旧口径复现 phase3 24 锚点为接线自检 (results/<ts>_E2_decomposition)"`，**不 push**。把脚本 `scp` 回本地 `_remote_tmp/e2_decomposition.py` 留档（与各 phase 脚本同处）。
- `exec_briefs/E2_REPORT.md`（结构见 `00_协议.md`）：起点确认 4 项；`check.csv` 32 行原样；`decomp.csv` 44 行（可直接贴 CSV 文本，保留 4 位小数）；日志里 `VETO MARGINAL` 与 `CANDIDATES` 两张表原样；G2-4 两行；耗时；results 目录、日志路径、commit 哈希；纯文本摘要 ≤20 行（六候选 old→new 的四段 net 与 NW、veto 边际 old→new、三腿与裸底池的新口径四段、detS 的 d_timing）；BLOCKERS。
- WORKLOG 一条（远端 + 本地 `_tmp_WORKLOG.md` 同文）：跑了什么 / 结果目录 / 硬软自检结果 / 关键数字（六候选新口径四段 net、veto 边际新口径）/ commit / 红线（地基未动、交付池未重出、未 push）/ 客观结论一句（只陈述数字方向，不下"保留/退役"判断）/ 状态：交规划 session 判。

## 5. 禁区
四个地基文件不动；不改不跑交付脚本与 phase 脚本；不改 `comprehensive_factor_diagnosis.py`；不改 CFGS / COMBOS / 锚点 / TOL；不重出交付池、不碰公共库；不延长窗口；不删 results；不 push；不在 REPORT 里替用户下"可以交付 / 该退役"的结论；结果数字不外发；文档与日志不出现登录信息与主机地址。凡遇"必须改上述任何一项才能继续" → 停，写 BLOCKERS，交回。
