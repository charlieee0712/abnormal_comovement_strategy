# E4 —— 聚合层复审：新口径下 reversal 席位 / 反转代表 / CVR 新轴 / cmf 正向 veto + 双基准 + 相关矩阵

用途：E3 单因子表出来后，规划 session 完成了逐因子复审（判读见 memory 入口条目 §7）。需要翻案或换代表的都在聚合层：reversal 的席位、反转代表的替换、CVR_20d 这条"日内强势反转"轴（旧口径下被误退役）进合成池还是进规则池、cmf 按正确方向作 veto。本步把这些配置在**同一新口径、同一 pipeline**下并排跑出来，附主副双基准和 8 因子相关矩阵。只出表，不判定。前置：`E3_REPORT.md`（BLOCKER-1 已由规划 session 按选项 1 放行，见 §0）。先读 `00_协议.md`。

## 0. 背景（读懂再动手）
- **E3 BLOCKER-1 处置 = 选项 1（放行）**：reversal 在 E2/phase 链路是"中性化后取负再剔组 2"，E3 是"不取负剔组 1"，差异只在 k=2 奇数日的中位那一只，两边各自可复现。**本步全部沿用生产构造**（取负再剔组 2），因此本步的 M_mean3/A4b/M_union3 必须逐格复现 E2 的 L1A 数字（硬锚点），而不是 E3 表里的 rev 行。
- 三个 commit 已在 origin（`origin/main = 2f23186`，2026-09-02 核实）。本步 commit 后照常 push。
- 口径与 E2 完全一致：I11 pool0（mcap=0）内 mcap 中性化 → 分组剔尾 → DEV 偏离约束权重 → hold 5 → 6bp → 引擎默认 `(exec_lag=1, adjust=True)`。
- **双基准**：主 = 干净全市场等权（E2 同，可与 E2/E3 对齐）；副 = **pool0 DEV 同机制**（把 pool0 全体按同一 DEV 权重、同一 rolling/lag 跑成一条"池自身"日收益，每单位仓位的收益作基准；候选超额₂ = 组合日收益 − 池单位收益 × 组合仓位，再扣同样成本）。副基准读的是"剔尾在池内的选股能力"，与 E2 里 pool0_DEV 早段 −5.2%/年的池拖累分离。
- **配置 13 个**（`CFGS` 顺序）：
  1. `A4b` / `M_mean3` / `M_union3`：生产三候选，**硬锚点 = E2 decomp.csv 的 L1A**（12 格，tol 0.02，任一 FAIL → ABORT）。
  2. `M_mean2`：cond+tvol 两因子 mean 剔上半 ≡ A组 A5（2 因子 median≡mean）；**软锚点 = phase3 A5 的旧口径**（用 `(0, False)` 多跑一次对 6.4231/6.5845/6.5189/6.9157，WARN 不阻塞）。用途：`M_mean3 − M_mean2` = reversal 席位边际。
  3. `M_mean3_cr5` / `M_mean3_cr20`：M_mean3 里的 reversal_skip1 换成 cum_return_5d / cum_return_20d（原值高=过去赢家=worst，不取负）。用途：反转代表替换。
  4. `M_mean4_cvr`：cond+tvol+rev+CVR_20d 四因子 mean 剔上半（CVR 原值高=worst）。用途：CVR 进合成池。
  5. `A4b_CVRv5` / `M_mean3_CVRv5` / `M_union3_CVRv5`：剔 CVR_20d 最高 quintile 作 veto。用途：CVR 进规则池。
  6. `A4b_CMFsurge5` / `M_mean3_CMFsurge5` / `M_union3_CMFsurge5`：剔 **cmf_change 激增** quintile（= `cmf_change_neg` 最低组，`drop_groups=[1]`）作 veto。注意方向：生产 CMFv5 剔的是 `cmf_change_neg` 最高组（资金流转弱），E3 形状显示那反而是池内最好的一组；本步测的是原经济故事"资金流加速→短期见顶"的方向。
- **相关矩阵**：8 个因子（cond / tvol / rev_neg / cr5 / cr20 / cvr / ci5=cum_intraday_ret_5d / cmf_neg）中性化后的池内 pct 值，逐日截面 Pearson（pct 上的 Pearson = Spearman），按天平均，每段一张。用途：CVR 与三腿的稀释检验（阈 0.5）、反转家族/日内家族内部冗余。
- **规划 session 事前定的判据（2026-09-02，写在这里是为了留时间戳，执行 session 不用它判）**：
  - reversal 席位保留：`M_mean3 − M_mean2` 主基准 net ≥3 段非负，2024-26 不差于 −0.5，NW ≥3 段不低。
  - 换反转代表：变体较 M_mean3 net ≥3 段更高且 NW ≥3 段更高，2024-26 不更差。
  - CVR 进合成池：`M_mean4_cvr − M_mean3` net ≥3 段 >0、2024-26 不降、NW ≥3 段升，且 CVR 与 cond/tvol/rev 相关 |ρ| < 0.5。
  - CVR / cmf 作 veto：净边际 ≥3 段 >0 **且** NW ≥3 段升 **且** 无一段降超 1.0（比 Phase-3 的"不实质变差即保留"严，那条放进了假象）。
  - 合成与 veto 都过 → 取近两段 NW 更高者。
- 预计 15–25 分钟（单进程顺序 4 段：每段 9 次中性化 + 13 配置 + 相关矩阵）。超过 45 分钟报告。

## 1. 起点确认（任一不符 → 停，写 BLOCKERS）
1. `E3_REPORT.md` 存在；47 上 `git log -1 --oneline` = `2f23186`，`git status -sb` 首行 `## main...origin/main`（已同步）；untracked 仍只有那 3 个。
2. `python -c "import comprehensive_factor_diagnosis as C; s={x['name'] for x in C.get_default_factor_specs()}; print(C.compute_calendar_pnl.__defaults__, {'CVR_20d','cum_return_5d','cum_return_20d','cum_intraday_ret_5d'} <= s)"` = `(5, 6, 1, True) True`。
3. 四个分段缓存在；`ps` 无本项目残留；负载 <5。

## 2. 新建 `e4_agg_review.py`（project_core 根目录，import-only，只读地基）

完整脚本如下。**pipeline 段与 helper 逐字取自 E2；允许修语法类小错，不得改 CFGS / 锚点 / TOL / 构造方向。**
...
python -c "import ast; ast.parse(open('e4_agg_review.py').read()); print('syntax OK')"
nohup taskset -c 96-191,288-383 python -u e4_agg_review.py > /tmp/E4_agg.log 2>&1 &
```
预计 15–25 分钟；超过 45 分钟无 `[ALLDONE]` / `[ABORT]` → `ps` 看清楚后报告。

## 3. 验收
- **G4-1 硬锚点**：12 行 `[CHECK HARD] … OK`，`[SELF-CHECK] HARD 12/12 PASS`。E2 同构造，应当 `|d|0.0000`；有 0<|d|<0.02 也算过但单列说明。FAIL → 脚本自行 ABORT，REPORT 写 BLOCKERS，**不改锚点、不改 TOL、不改构造**。
- **G4-2 软锚点**：4 行 `[CHECK SOFT]`（M_mean2 旧口径 vs phase3 A5），OK 或 WARN 都原样摘进 REPORT，WARN 不阻塞（A5 的原构造若不是 2 因子 mean，就会 WARN，那是信息不是故障）。
- **G4-3 产物**：`summary.csv`（13 × 4 + 4 = 56 行）、`check.csv` 16 行、`corr_<段>.csv` 4 张、`log.txt` 含 MAIN / MARGINALS 表与 4 张相关矩阵。
- **G4-4 合理性（只报告）**：日志每段的 `pool0_DEV vs market` 行应与 E2 `pool0_DEV` L1A 四段一致（−5.22/−2.86/−1.93/−0.35，|d|<0.02）；把这 4 个数列出。

## 4. 交回
- 47 上 `git add e4_agg_review.py && git commit -m "E4 聚合复审脚本: 新口径下 rev 席位/反转代表/CVR 新轴(合成+veto)/cmf 正向 veto, 双基准(全市场等权+pool0 同机制) + 8 因子相关矩阵 (results/<ts>_E4_agg_review)"` → `git push origin main`。`scp` 回本地 `_remote_tmp/e4_agg_review.py`。
- `exec_briefs/E4_REPORT.md`：起点确认 3 项；G4-1/2/3/4；`check.csv` 16 行；日志 MAIN 表、MARGINALS 表、4 张相关矩阵**原样**；`summary.csv` 56 行（保留 4 位小数）；耗时；results 目录、日志路径、commit、push 结果；纯文本摘要 ≤20 行（只写数字与方向，**不写"保留/退役/进池"判断**）；BLOCKERS。
- WORKLOG 一条（远端 + 本地 `_tmp_WORKLOG.md` 同文）：跑了什么 / 结果目录 / 锚点结果 / commit + push / 红线 / 客观结论一句（只陈述表已出）/ 状态：交规划 session 判。

## 5. 禁区
四个地基文件不动；不改 `comprehensive_factor_diagnosis.py`；不改 CFGS / 锚点 / TOL / 各配置的方向与构造；不延长窗口；不重出交付；不删 results；不在 REPORT 里下判断；结果数字不外发；文档与日志不出现登录信息与主机地址。凡遇"必须改上述任何一项才能继续" → 停，写 BLOCKERS，交回。
