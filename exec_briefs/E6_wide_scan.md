# E6 —— 宽扫描 v1：成本 8bp + 形状角色表 + 合成层深度曲线 + 否决层全因子扫描 + 配对统计（只报告，不淘汰）

用途：这是引擎修正后重建组合的第一步。**探索从宽**（用户 2026-09-03 定规，见 memory `exploration-loose-admission-strict`）：不预筛、不按单因子强弱淘汰、深度和方向多档并排，全部报出；任何"采纳 / 淘汰"都不在本步发生，留给延长样本（E7）和用户过目。本步产出一张可排序的大表和一张形状角色表，规划 session 与用户一起看，再定 E6b（叠加、主题 sleeve）。前置：`E5b_REPORT.md`（commit `db7a0af`）。先读 `00_协议.md`。

## 0. 背景（读懂再动手）
- **成本统一 8bp**（用户 2026-09-03 定）：一次买卖合计，含义 = 印花税 5bp + 佣金约 3bp，不含滑点。引擎默认值、模块常量、`--cost_bp` 默认一并改 8。E0–E5a 的脚本都显式传 6，其结果目录与锚点不受影响；`selftest_engine_fix.py` 也全部显式传值，改完重跑应仍 5/5。v2 交付 README 里的 6bp 数字**本步不重生成**，等最终形态定稿后一次按 8bp 重出。
- **形状统计**：从 E3 的 `shape.csv` 算 `tail_share = (次差组 − 最差组) / (最好组 − 最差组)`，四段 g1..g5 先取均值再算。低 = 五组渐变（适合取百分位平均），高 = 只有一端坏（适合作否决）。这张表由脚本算、写进 `roles.csv`，轴标签由规划 session 手工给（脚本里的 `AXIS` 字典，是初判，供读表用）。
- **方向**：由形状定。四段汇总的最差组在 g5 → 剔高；在 g1 → 剔低；在 g2–g4（形状不清）→ **两个方向都测**。E3 已经双向测过全部 34 个，这里不丢信息。
- **两个底座**：`A4b`（生产，cond→tvol 依赖双排序，保留约 1/4）与 `M_mean2`（cond+tvol 独立均值，剔一半）。`M_mean3_v2`（含 cr20）出现在深度曲线里，并作 `M_mean2` 的配对对照（= reversal 席位边际）。
- **配对统计**：每个配置对其底座的**逐日** Δnet 序列算 NW t（每段 + 全窗口）。这是比"两条序列各自的 t"更对的比法（Ledoit-Wolf 2008 配对思路）。**只作列**，用来排序；不作闸门。
- **建议清单**：脚本按"全窗口配对 t > 1.0 且 ≥2/4 段 Δnet > 0"打一个 `watch` 标记，那是**建议**，用户可任意增删。
- 预计 4 段并行 wall 15–30 分钟（用户已知悉）；超过 60 分钟报告。

## 1. 起点确认（任一不符 → 停，写 BLOCKERS）
1. 47 上 `git log -1 --oneline` = `db7a0af`；`git status -sb` 首行 `## main...origin/main`；untracked 仍只有那 3 个。
2. `python -c "import comprehensive_factor_diagnosis as C; print(C.compute_calendar_pnl.__defaults__, C.COST_BP_BILATERAL)"` = `(5, 6, 1, True) 6.0`（改前）。
3. `results/20260903_1037_E3_single_factor/shape.csv` 在，140 行 + 表头；四个分段缓存在；`ps` 无本项目残留；负载 <5。

## 2A. 成本 8bp（先做，单独 commit）

`comprehensive_factor_diagnosis.py` 四处，其余一字不动：
- 第 35–36 行：`# 双边成本 (bp), 可被 --cost_bp 覆盖. 默认6 (与历史口径一致).` / `COST_BP_BILATERAL = 6.0` → `# 成本 (bp, 一次买卖合计), 可被 --cost_bp 覆盖. 2026-09-03 起默认 8 = 印花税 5 + 佣金约 3, 不含滑点; 2026-09 之前的结果目录为 6.` / `COST_BP_BILATERAL = 8.0`
- 第 488 行签名：`cost_bp_bilateral=6` → `cost_bp_bilateral=8`
- 第 528 行注释：`(可配置, 默认6bp)` → `(可配置, 默认 8bp; 2026-09-03 起)`
- 第 1036–1037 行：`default=6.0` → `default=8.0`；help 文案 `'双边成本 bp (默认6, 可调; 实盘中低频参考10-15)'` → `'成本 bp, 一次买卖合计 (默认 8 = 印花税 5 + 佣金 3; 含滑点约 12)'`

`PROJECT_STATUS.md` §4.6 第一条（第 126 行）整行替换为：
`- **2026-09-03 起统一 8bp**（一次买卖合计：印花税 5bp + 佣金约 3bp，不含滑点；引擎 `compute_calendar_pnl` 默认值、`COST_BP_BILATERAL`、`--cost_bp` 同改）。E0–E5a 的结果目录与锚点仍是 6bp（脚本显式传 6，历史可比不受影响）；v2 交付 README 的 6bp 数字待最终形态定稿后按 8bp 重出。含滑点按 12bp 为保守档；2023-08 之前印花税为 10bp，早段历史实盘成本更高。`

然后：
```bash
python -c "import ast; ast.parse(open('comprehensive_factor_diagnosis.py').read()); print('syntax OK')"
python -c "import comprehensive_factor_diagnosis as C; print(C.compute_calendar_pnl.__defaults__, C.COST_BP_BILATERAL)"   # 期望 (5, 8, 1, True) 8.0
nohup taskset -c 96-191,288-383 python -u selftest_engine_fix.py > /tmp/E6_selftest.log 2>&1 &     # 约 1 分钟, 期望 [SELFTEST] 5/5 PASS, 五条数字与 E1 逐位相同
git add comprehensive_factor_diagnosis.py PROJECT_STATUS.md && git commit -m "cost: 引擎默认成本 6→8bp (一次买卖合计=印花税5+佣金3, 不含滑点; 用户 2026-09-03 定) + §4.6; 历史脚本显式传 6 不受影响; selftest 5/5" && git push origin main
```
selftest 不是 5/5 或任一数字与 E1 不同 → BLOCKER，停。

## 2B. 新建 `e6_wide_scan.py`（project_core 根目录，import-only，只读地基）

完整脚本如下。**pipeline 与 helper 逐字取自 E4/E5a；允许修语法类小错；不得改 `KEEPS / KS_VETO / BASES / 方向规则 / 锚点 / 成本值`。`AXIS` 字典是规划 session 的手工初判，照抄。**
...
python -c "import ast; ast.parse(open('e6_wide_scan.py').read()); print('syntax OK')"
python -c "
import sys; sys.argv=['x','--merge','--out','/tmp/e6_roles_check']; exec(open('e6_wide_scan.py').read().split('def run_period')[0]); r,d=roles_table(); print(r[['factor','axis','tail_share','worst_pooled','dirs_tested','role_hint']].to_string(index=False)); print('n dirs both:', sum(len(v)==2 for v in d.values()))"   # 只读预检: 35 行, 无 '?' 轴
OUT=/mnt/sda2/lichenchen/results/$(date +%Y%m%d_%H%M)_E6_wide_scan; mkdir -p $OUT; echo $OUT
for p in 2010-2014 2015-2018 2019-2023 2024-2026; do
  OMP_NUM_THREADS=16 MKL_NUM_THREADS=16 OPENBLAS_NUM_THREADS=16 \
  setsid nohup taskset -c 96-191,288-383 python -u e6_wide_scan.py --period $p --out $OUT > /tmp/E6_$p.log 2>&1 &
done
# 轮询 grep -l 'PERIOD DONE' $OUT/log_*.txt | wc -l == 4 (间隔 ≥120 s); 任一 log 出现 [ABORT] 即停
python -u e6_wide_scan.py --merge --out $OUT > $OUT/log_merge.txt 2>&1; tail -2 $OUT/log_merge.txt
```

## 3. 验收（全是"跑齐了没有"，没有"过没过"）
- **G6-1 锚点**：4 段各 3 行 `[ANCHOR 6bp] … OK`，共 12，`|d|0.0000`。任一 FAIL → 脚本自行 ABORT，REPORT 写 BLOCKERS，不改锚点。
- **G6-2 齐全**：4 个 `[PERIOD DONE]`；每段 rows = 3 锚点 + 8 底座 + 14 深度 + veto 配置数；`roles.csv` 35 行、无 `?` 轴；`scan_all.csv` 行数 = 8 + 14 + veto 配置数；`daily_all.parquet` 列数同；`[MERGE DONE]`。veto 配置数 = Σ_f len(dirs_f) × 4 × 2，从预检的 "n dirs both" 推算并核对。
- **G6-3 8bp 对账**：`A4b` 8bp 四段 net 应比 6bp 锚点低约 `turn × 2/100`（如 turn 30 → 约 0.6）；把 8 个底座行的 6bp→8bp 差列出来（三个有 6bp 锚点的直接比，其余按换手估）。
- **G6-4 selftest**：2A 的 `[SELFTEST] 5/5 PASS`，五条数字与 E1 逐位相同。
- 耗时每段 + 最慢因子写进 REPORT。

## 4. 交回
- commit 2（扫描脚本）：`git add e6_wide_scan.py && git commit -m "E6 宽扫描脚本: 形状角色表 + 合成层深度曲线 + 35 因子否决层全扫描 (方向由形状定, k3/4/5/10, 双底座) + 配对 NW t 列; 8bp; 6bp 锚点接线自检 (results/<ts>_E6_wide_scan)" && git push origin main`。`scp` 回本地 `_remote_tmp/e6_wide_scan.py`。
- `exec_briefs/E6_REPORT.md`：起点确认；2A 四处改动落点 + selftest 结果 + commit 1；G6-1～4；`summary.txt` 的 (A)(B)(C) 三段**原样全文**，(D) 每底座前 30 行 + (D2) 全部 + watch 计数；每段耗时；results 目录、日志、两个 commit、push；纯文本摘要 ≤15 行（只写规模、锚点、8bp 对账、watch 数、深度曲线的形状描述，**不写任何"该进 / 该退"**）；BLOCKERS。
- WORKLOG 一条（远端 + 本地 `_tmp_WORKLOG.md` 同文）：成本改动 + 扫描内容 / 结果目录 / 锚点 / 两个 commit + push / 红线 / 客观结论一句（"表已出，未做任何淘汰"）/ 状态：交规划 session 与用户共读。

## 5. 禁区
四个地基文件不动；引擎只改 2A 列出的四处；不改 `KEEPS / KS_VETO / BASES / 方向规则 / 锚点 / 成本值 / AXIS`；不重出 v2 交付；不碰公共库；不删 results；REPORT 里不下任何采纳 / 淘汰判断；结果数字不外发；文档与日志不出现登录信息与主机地址。凡遇"必须改上述任何一项才能继续" → 停，写 BLOCKERS，交回。
