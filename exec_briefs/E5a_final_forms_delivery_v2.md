# E5a —— 终形态复现 + 交付文件 v2（进 results/ 与 git delivery/，不碰公共库）

用途：E4 之后规划 session 已定终判（memory `resonance-engine-fix-readjudication-verdict`）：合成池 = cond + tvol + **cum_return_20d**（反转代表由 reversal_skip1 换为 cr20），规则池 = **CVR_20d 窄 veto（k=5）**取代 cmf veto，cmf_change 退役。本步把六个 v2 形态在新口径下一次跑齐、自检对 E4 锚点、导出与 7/8 同格式的交付文件（池1 + 6 个池2），并生成 README。**只写 results/ 与仓库 `delivery/`，不复制到公共库**（公共库交付与周报是 E5b，等用户看过数字）。先读 `00_协议.md`。

## 0. 背景（读懂再动手）
- 六形态 v2 = {`A4b`, `M_mean3_v2`, `M_union3_v2`} × {纯净, `+CVRv5`}。A4b 不含反转，与 v1 相同；`M_mean3_v2` = cond/tvol/cr20 三个池内 pct 的 mean 剔上半；`M_union3_v2` = 三因子各自 drop 的并集之外；`CVRv5` = 剔池内 CVR_20d 最高 quintile。cr20、CVR 都是"原值高 = worst"，**不取负**。
- 硬锚点（E4 `results/20260903_1132_E4_agg_review/summary.csv` L1A，tol 0.02，任一 FAIL → ABORT 不写交付文件）：`A4b` = 2.5046/5.0447/7.5665/8.9262；`A4b_CVRv5` = 3.0584/6.9892/8.4299/10.3293；`M_mean3_v2`（= E4 的 `M_mean3_cr20`）= 3.5462/7.0335/5.5572/6.3298。其余三形态本步首次出数。
- 池1（I11 初筛）不依赖因子，导出后 md5 必须等于 7/8 那份 `6006780eb791fe6a7f66e3829cb23763`。
- 交付格式沿用 7/8：`ticker/tradeDate/weight` 三列长表，ticker 去后缀去前导零整数，weight = DEV 偏离约束目标权重（不归一，日权重和 = 当日仓位），全历史 2010-02-12～2026-03-27。
- 口径：新口径 `(exec_lag=1, adjust=True)`，DEV，hold 5，6bp，主基准干净全市场等权，副基准 pool0 DEV 同机制（E4 同）。窗口不变。
- 预计 5 分钟。

## 1. 起点确认（任一不符 → 停，写 BLOCKERS）
1. `E4_REPORT.md` 12/12；47 上 `git log -1 --oneline` = `54c5291`，`git status -sb` 首行 `## main...origin/main`；untracked 仍只有那 3 个。
2. 引擎默认值 `(5, 6, 1, True)`；`results/20260903_1132_E4_agg_review/summary.csv` 与 `results/20260903_0935_E2_decomposition/decomp.csv` 都在（脚本要读）。
3. 四个分段缓存在；`ps` 无本项目残留；负载 <5。

## 2. 新建 `export_delivery_pools_v2.py`（project_core 根目录，import-only，只读地基）

完整脚本如下。**pipeline 与 helper 逐字取自 7/8 交付脚本 / E2；允许修语法类小错，不得改形态定义、锚点、TOL、输出格式。**
...
lines=[]
lines.append('OLD vs NEW  DEV net ann %% (NW)  4 段 = %s'%' / '.join(SEGS))   # 2026-09-03 执行 session 修正: 裸 % 须写 %%
...
python -c "import ast; ast.parse(open('export_delivery_pools_v2.py').read()); print('syntax OK')"
nohup taskset -c 96-191,288-383 python -u export_delivery_pools_v2.py > /tmp/E5a_export.log 2>&1 &
```
预计 5 分钟；超过 30 分钟报告。

## 3. 验收
- **G5-1 锚点**：12 行 `|d|0.0000`（E4 同构造应逐位相同），`[SELF-CHECK] ALL 12 PASS`。FAIL → 脚本自行 ABORT，REPORT 写 BLOCKERS，不改锚点。
- **G5-2 池1**：日志 `md5=6006780eb791fe6a7f66e3829cb23763 OK(==0708)`。MISMATCH → BLOCKER（说明 pipeline 漂移），文件保留但不进 git。
- **G5-3 产物**：7 个 CSV、`_delivery_stats.csv`（6 行，`wmax` 均 ≤ 0.010000）、`summary.csv` 24 行、`check.csv` 12 行、`old_vs_new.txt`、`README.md`（表格 6 行数字齐、无 `nan`）。
- **G5-4 只报告**：三个首次出数的形态（`M_union3_v2`、`M_mean3_v2_CVRv5`、`M_union3_v2_CVRv5`）四段 net1/NW1/net2/NW2 单独摘出。

## 4. 交回
- **git（仓库内交付目录，与 7/8 同法）**：
  ```bash
  cd /mnt/sda2/lichenchen/code/project_core
  D=delivery/I11_candidate_pools_v2_20260903; mkdir -p $D
  R=<results 目录>
  tar -czf $D/I11_candidate_pools_v2_20260903.tar.gz -C $R pool1_I11_screening.csv pool2_A4b_pure.csv pool2_A4b_cvrveto.csv pool2_Mmean_v2_pure.csv pool2_Mmean_v2_cvrveto.csv pool2_Munion_v2_pure.csv pool2_Munion_v2_cvrveto.csv
  cp $R/README.md $R/_delivery_stats.csv $R/summary.csv $R/old_vs_new.txt $D/ ; cp export_delivery_pools_v2.py $D/
  ls -la $D   # tar.gz 约 10M 量级
  git add export_delivery_pools_v2.py $D && git commit -m "delivery v2: 引擎修正后六候选形态 (cond+tvol+cum_return_20d 合成 / CVR_20d veto k5) 池1+6 池2 gzip 归档 + README + 复现脚本 (results/<ts>_delivery_pools_v2); 自检 E4 锚点 12/12, 池1 md5 == 0708" && git push origin main
  ```
  **不复制到公共库** `/mnt/big/base/public/...`（E5b 用户确认后再做）。脚本 `scp` 回本地 `_remote_tmp/export_delivery_pools_v2.py`。
- `exec_briefs/E5a_REPORT.md`：起点确认；G5-1～4；`check.csv` 12 行；`old_vs_new.txt` 全文；`_delivery_stats.csv` 6 行；README 里的六形态表原样；results 目录、日志、commit、push；纯文本摘要 ≤15 行；BLOCKERS。
- WORKLOG 一条（远端 + 本地 `_tmp_WORKLOG.md` 同文）：跑了什么 / 结果目录 / 自检 / 交付文件与 git 路径 / 红线（未碰公共库、地基未动）/ 客观结论一句 / 状态：等用户审数字后 E5b。

## 5. 禁区
四个地基文件不动；不改引擎；不改形态定义 / 锚点 / TOL / 输出格式；不碰公共库；不改 v1 交付目录；不延长窗口；不删 results；不在 REPORT 里下"可交付"结论；结果数字不外发；文档与日志不出现登录信息与主机地址。凡遇"必须改上述任何一项才能继续" → 停，写 BLOCKERS，交回。
