...
SEED = 20260923
SEGCODE = {'2010-2014': 1, '2015-2018': 2, '2019-2023': 3, '2024-2026': 4}   # 后段 (Stage 3) 新段码 = 新 draw
BATCH = 64
...
def main():
    ...
    ap.add_argument('--npaths', type=int, default=0, help='加轮: 本次路径数 (覆盖 random_priority x paths_mult)')
    ap.add_argument('--only-list', default='', help='加轮: 只跑该 csv (descriptor_id 列) 里出现的配置')
    ap.add_argument('--tag-suffix', default='', help='输出文件名后缀 (避免与首轮分片同名)')
    a = ap.parse_args()
    ...
    for gi, k in enumerate(keys):
        ...
        ts = time.time()
        np_ = 8 if a.selftest else (a.npaths if a.npaths else int(r0['random_priority']) * a.paths_mult)
        try:
    ...
    tag = ('selftest_%s' % a.route) if a.selftest else ('%s_%02d' % (a.route, a.shard) + ('_timing' if a.limit else '') +
                                                       ('_x%d' % a.paths_mult if a.paths_mult > 1 else '') + ('_p%d' % a.path_start if a.path_start else '') + ('_n%d_%s' % (a.npaths, a.kinds.replace(',', '')) if a.path_start else '') + a.tag_suffix)
    d = pd.DataFrame(rows)
